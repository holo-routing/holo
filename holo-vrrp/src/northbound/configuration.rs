...
fn load_callbacks() -> Callbacks<Interface> {
    CallbacksBuilder::<Interface>::default()
        ...
        .create_apply(|interface, args| {
            ...
            let addr = IpNetwork::V4(args.dnode.get_prefix4_relative("ipv4-address").unwrap());
        ...
        .create_apply(|interface, args| {
            ...
            let addr = IpNetwork::V6(args.dnode.get_prefix6_relative("ipv6-address").unwrap());
            instance.config.virtual_addresses.insert(addr);
    ...
    fn process_event(&mut self, event: Event) {
        match event {
            Event::InstanceStart {
                ...
                let (interface, instance) = self.get_instance(vrid, af).unwrap();
            ...
            Event::InstanceDelete {
                ...
                let mut instance = match af {
                    AddressFamily::Ipv4 => self.vrrp_ipv4_instances.remove(&vrid).unwrap(),
                    AddressFamily::Ipv6 => self.vrrp_ipv6_instances.remove(&vrid).unwrap(),
                };
