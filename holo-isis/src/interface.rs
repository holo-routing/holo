...
pub struct InterfaceSys {
    ...
    // List of IPv4 addresses associated with this interface.
...
pub struct InterfaceState {
    ...
    // Circuit ID.
    pub circuit_id: u8,
    // Adjacencies attached to this interface.
    pub lan_adjacencies: Levels<Adjacencies>,
    pub p2p_adjacency: Option<Adjacency>,
    // Designated IS for the LAN.
    pub dis: Levels<Option<DisCandidate>>,
    // Lists of LSPs used by the IS-IS flooding algorithm.
...
impl Interface {
    ...
    pub(crate) fn dis_election(
        ...
        // Filter adjacencies that are eligible for DIS election (only those
        // in the "Up" state).
        let mut adjs = self
            .state
            .lan_adjacencies
    ...
    pub(crate) fn srm_list_add(&mut self, level: LevelNumber, mut lsp: Lsp) {
        // Don't add LSPs to the SRM list if there are no active adjacencies
        // or if the LSP has a sequence number of zero.
        if !self.state.active
            || self.state.event_counters.adjacency_number == 0
            || lsp.seqno == 0
        {
            return;
        }
        ...
        // On point-to-point interfaces, all LSPs must be acknowledged. If they
        // are not, retransmissions will be triggered until an acknowledgment
        // is received.
...
impl InterfaceType {
    // Returns the appropriate multicast address based on the interface type
    // and IS-IS level.
