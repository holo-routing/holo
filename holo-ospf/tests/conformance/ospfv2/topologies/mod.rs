...
#[tokio::test]
async fn topology_3_1() {
    for rt_num in 1..=6 {
        let rt_name = format!("rt{rt_num}");
        run_test_topology::<Instance<Ospfv2>>("topo3-1", &rt_name).await;
    }
}

#[tokio::test]
async fn topology_3_2() {
    for rt_num in 1..=6 {
        let rt_name = format!("rt{rt_num}");
        run_test_topology::<Instance<Ospfv2>>("topo3-2", &rt_name).await;
    }
}

#[tokio::test]
async fn topology_3_3() {
    for rt_num in 1..=6 {
        let rt_name = format!("rt{rt_num}");
        run_test_topology::<Instance<Ospfv2>>("topo3-3", &rt_name).await;
    }
}
