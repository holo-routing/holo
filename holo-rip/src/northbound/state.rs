    ...
    CallbacksBuilder::<Instance<V>>::default()
        ...
        .get_object(|instance, _args| {
            ...
            if let Some(instance_state) = &instance.state {
                next_triggered_update = instance_state.next_triggered_update().map(|d| d.as_secs().saturating_into());
        ...
        .get_object(|_instance, args| {
            ...
            let iface = args.list_entry.as_interface().unwrap();
            let discontinuity_time = iface.state.statistics.discontinuity_time.as_ref();
            let bad_packets_rcvd = Some(iface.state.statistics.bad_packets_rcvd);
            let bad_routes_rcvd = Some(iface.state.statistics.bad_routes_rcvd);
            let updates_sent = Some(iface.state.statistics.updates_sent);
            Box::new(Statistics {
                discontinuity_time: discontinuity_time.map(Cow::Borrowed).ignore_in_testing(),
        ...
        .get_object(|instance, _args| {
            ...
            if let Some(instance_state) = &instance.state {
                discontinuity_time = instance_state.statistics.discontinuity_time.as_ref();
...
fn load_callbacks_ripv2() -> Callbacks<Instance<Ripv2>> {
    ...
    CallbacksBuilder::new(core_cbs)
        ...
        .get_iterate(|instance, _args| {
            ...
            let iter = instance_state.neighbors.values().map(ListEntry::Ipv4Neighbor);
            Some(Box::new(iter))
        })
        .get_object(|_instance, args| {
        ...
        .get_object(|instance, args| {
            ...
            Box::new(Route {
                ...
                interface: instance.interfaces.get_by_ifindex(route.ifindex).map(|(_, iface)| iface.name.as_str().into()),
...
fn load_callbacks_ripng() -> Callbacks<Instance<Ripng>> {
    ...
    CallbacksBuilder::new(core_cbs)
        ...
        .get_iterate(|instance, _args| {
            ...
            let iter = instance_state.neighbors.values().map(ListEntry::Ipv6Neighbor);
            Some(Box::new(iter))
        })
        .get_object(|_instance, args| {
